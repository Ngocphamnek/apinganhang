...
export const notifyTelegram = async (message: string) => {
  ...
  try {
    ...
    await axios.post(url, { chat_id: settings.telegram.chatId, text: message, parse_mode: 'HTML' });
  } catch (error: any) {
...
export const notifyCustomWebhook = async (transaction: any) => {
  ...
  try {
    ...
    await axios.post(settings.customWebhook.url, { status: 'success', ...transaction }, { headers, timeout: 5000 });
  } catch (error: any) {
...
/**
 * Fire a targeted callback for a matched pending deposit.
 * This bypasses the global customWebhook setting and uses the registered callbackUrl directly.
 */
export const notifyMatchedDeposit = async (
  ...
  try {
    ...
    const resp = await axios.post(callbackUrl, payload, { headers, timeout: 8000 });
