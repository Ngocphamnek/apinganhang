    ...
    <template v-else-if="transactions.length">
      ...
      <div class="charts-row">
        <el-card class="chart-card">
          ...
          <v-chart ref="pieChartRef" class="chart" :option="pieOptions" />
        </el-card>
        ...
        <el-card class="chart-card">
          ...
          <v-chart ref="barChartRef" class="chart" :option="barOptions" />
        </el-card>
      </div>
...
// Chart refs for manual resize (avoids ECharts autoresize throwing non-Error objects)
...
const safeResizeCharts = () => {
  try { (pieChartRef.value as any)?.chart?.resize(); } catch { /* ignore */ }
  try { (barChartRef.value as any)?.chart?.resize(); } catch { /* ignore */ }
};
...
const barOptions = computed(() => {
  ...
  return {
    ...
    grid: { left: '60px', right: '20px', bottom: '15%', top: '5%' },
    ...
    series: [
      { name: 'Income', type: 'bar', stack: 'total', itemStyle: { color: '#67c23a' }, data: inData },
      { name: 'Expense', type: 'bar', stack: 'total', itemStyle: { color: '#f56c6c' }, data: outData }
    ]
  };
});
